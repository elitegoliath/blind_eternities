...
class QueryClassification(BaseModel):
    ...
    entities: List[str] = Field(default_factory=list, description="List of card names, mechanics, or keywords extracted.")

def classify_query(user_prompt: str) -> QueryClassification:
    llm = get_llm().with_structured_output(QueryClassification)
    prompt = ChatPromptTemplate.from_messages([
        ("system", "You are an MTG rules classifier. Classify the user's input into one of three intents:\n"
