...
HEADERS = {"User-Agent": "BlindEternities/1.0", "Accept": "application/json"}
...
def download_if_needed(target_type):
    ...
    # 2. Find target
    target_info = next(
        (item for item in bulk_data if item["type"] == target_type), None
    )
    if not target_info:
...
def test_ingestion():
    ...
    cursor.execute(
        "SELECT id, oracle_id, name, type_line, oracle_text FROM cards WHERE name = ?",
        (test_card,),
    )
    ...
    oracle_id = card[1]
    cursor.execute(
        "SELECT date, text FROM rulings WHERE oracle_id = ? ORDER BY date ASC",
        (oracle_id,),
    )
